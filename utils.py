import json
from pyspark.sql.functions import create_map, lit, split, to_date, col, date_format, broadcast, row_number, when, current_date, year
from pyspark.sql.window import Window


def load_state_map():
    with open("us_state_abbr.json", "r", encoding="utf-8") as file:
...
def standardize_df(df, state_map):
    name_arr = split(df.name, " ")
    df = (
        df
        # Split full names into first_name and last_name
        .withColumn("first_name", name_arr.getItem(0))
        .withColumn("last_name", name_arr.getItem(1))
        .drop("name")
...
def validate_df(df):
    df = (
        df.withColumn("dob", to_date(col("dob"), "yyyy/MM/dd"))
